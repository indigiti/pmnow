...
<?php if ($live): ?>
<div class="live-controls"><button class="btn red" data-follow="<?= pm_e($story['id']) ?>"><?= $followed?'Following':'Follow live' ?></button><?php if ((string)pm_env('APP_ENV','local') !== 'production'): ?><button class="btn outline" data-live-demo="<?= pm_e($live['id']) ?>">+ Demo update</button><?php endif; ?></div>
