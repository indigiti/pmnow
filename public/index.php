...
$storyService=new StoryService($storyRepo,$mediaRepo,$store);$userService=new UserStateService($userRepo);$analyticsService=new AnalyticsService($store);$seoService=new SeoService($config);$personalizationService=new PersonalizationService($storyService,$userService);
...
$page=new PageController($view,$storyService,$userService,$notificationService,$store,$seoService,$personalizationService);$api=new ApiController($storyService,$userService,$store,$runtime,$config,$searchService,$notificationService,$analyticsService,$personalizationService);$discovery=new DiscoveryController($storyRepo,$storyService,$store);$hub=new HubApiController($sourceService,$hubService,$python,$authService);$adminPage=new AdminPageController($view,$authService,$editorialService,$sourceService,$jobRepo,$store,$errors,$healthService,$analyticsService);$newsroom=new NewsroomApiController($authService,$editorialService,$jobRepo,$schedulerService,$workerService,$searchService,$notificationService,$userService,$errors,$healthService);$router=new Router();
